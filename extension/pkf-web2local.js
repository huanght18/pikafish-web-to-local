...
(() => {
  ...
  const CONNECTION_TAG_ID = "pkf-web2local-connection-tag";
  const CONNECTION_TAG_STATES = {
    loaded: {
      text: "桥接已加载",
      title: "pkf-web2local 已加载，等待本地引擎初始化",
  ...
  function createBridge(onLine, onFallback) {
    ...
    const switchToWasm = (reason, replayPending) => {
      ...
      state = "wasm";
      clearTimeout(timer);
      console.warn(`[pkf-web2local] ${reason}; using webpage WASM for this engine session`);
    ...
    try {
      ...
      console.warn("[pkf-web2local] cannot create websocket:", error);
      state = "wasm";
      setConnectionTagState("disconnected");
    ...
    if (socket) {
      ...
      socket.onopen = () => {
        ...
        clearTimeout(timer);
        console.log("[pkf-web2local] ws connected:", WS_URL);
        const queued = pending.splice(0);
        try {
          for (const command of queued) socket.send(command);
        } catch (error) {
          console.warn("[pkf-web2local] flushing queued commands failed:", error);
          switchToWasm("local connection failed", false);
        }
      };

      socket.onerror = (error) => console.warn("[pkf-web2local] ws error:", error);
      socket.onclose = () => {
        if (state === "connecting") {
    ...
    return {
      send(command) {
        ...
        try {
          ...
          console.warn("[pkf-web2local] ws send failed:", error);
      ...
      Promise.resolve(result).then((engine) => {
        ...
        const sendToWasm = (command) => {
          try { origSend?.(command); } catch (error) {
            console.warn("[pkf-web2local] wasm sendCommand failed:", error);
          }
        };
        ...
        const bridge = createBridge(
          (line) => {
            ...
            try { uiStdout?.(line); } catch (error) {
              console.warn("[pkf-web2local] uiStdout error:", error);
            }
          },
          (queued) => {
        ...
        engine.sendCommand = (value) => {
          ...
          if (!command) return;
          console.log("[pkf-web2local] < " + command);
        ...
        console.log("[pkf-web2local] wasm engine sendCommand patched -> local ws");
      }).catch((error) => console.warn("[pkf-web2local] engine initialization failed:", error));
  ...
  Object.defineProperty(window, "Pikafish", {
    ...
    set(value) {
      ...
      console.log("[pkf-web2local] window.Pikafish set -> wrapped:", pikafishValue);
    },
  });

  console.log("[pkf-web2local] installed (document-start).");
})();
