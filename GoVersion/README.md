...
`servepkf2.py` 的 Go 重写版，行为与 Python 版**完全对齐**：
...
油猴脚本 `pkf-local.js` **无需任何改动**，因为协议沿用"一行一条 text message"。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `host` | string | WS 监听地址。本机用 `localhost`；不要改成 `0.0.0.0`，避免被同网段访问 |
| `port` | int | WS 监听端口，需与 `pkf-local.js` 中 `WS_URL` 一致 |
| `engine_path` | string | Pikafish 引擎绝对路径（**必填**），示例：`D:\Personal\ChineseChess\pikafish-20260131\pikafish-bmi2.exe` |
...
exe 首次启动会在同目录生成 `config.json`，编辑后再次双击即生效。
...
D:\GoPath\bin\rsrc.exe -ico "..\bitbug_favicon_x128.ico" -o pkf-local-go.syso
...
| `if "setoption name Hash value" in msg` | `strings.Contains(line, "setoption name Hash value")` |
| `proc.terminate()` | `engine.Kill()`（内含 `Process.Kill` + `cmd.Wait`） |
| `asyncio.gather` 并发 | 两个 goroutine + `sync.WaitGroup` + `context` |
...
- `gorilla/websocket` 的 `Upgrader.CheckOrigin` 默认拒绝浏览器 WebSocket；本工具是本机服务，置 `return true` 即可。

## 与 `pkf-local.js` 的协议契约
...
- **油猴脚本报 `ws not ready, fallback to wasm`**：本服务未启动 / 端口不一致 / 浏览器拦截 WS。检查 `[Go] ws server listening on ws://...` 是否出现。
